...
const PokemonInformations = ({route, item}) => {
  const dispatch = useDispatch();
  const toggleFavorites = array =>
    dispatch(AuthReducerFunctions.toggleFavorites(array));
  ...
  const renderCardsType = ({item}) => (
    <CardsType text={item.text} color={item.color} imageData={item.image} />
  );

  const renderCardsAbility = ({item}) => (
    <CardsAbility
      number={item.number}
      effect={item.effect}
  ...
  const Favoris = ({onPress}) => {
    return isFavorites() ? (
      <Button title={'Retirer des favoris'} onPress={() => onPress()} />
    ) : (
      <Button title={'Ajouter aux favoris'} onPress={() => onPress()} />
    );
  };
  ...
  return (
    <ScrollView nestedScrollEnabled={true}>
      ...
      <SafeAreaView>
        ...
        <Button title={'Partager'} onPress={() => _getContacts(details.name)} />
      </SafeAreaView>
    </ScrollView>
  );
};
